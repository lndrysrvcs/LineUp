...
public class CRUDTests
{
    ...
    /*
    [Fact]
    public async Task CreateRandomAvailability_Test()
    {
        // Arrange
        var options = new DbContextOptionsBuilder<LineUpContext>()
            .UseInMemoryDatabase(databaseName: Guid.NewGuid().ToString())
        ...
        List<DateTime> randomAvailSlots = new List<DateTime>();

        for (int i = 0; i < totalDays; i++)
        {
            randomAvailSlots.Add(DateTime.UtcNow.Date.AddHours(9));
            DateTime.UtcNow.Date.AddHours(9).AddMinutes(30);
            DateTime.UtcNow.Date.AddHours(10);
            DateTime.UtcNow.Date.AddHours(10).AddMinutes(30);
            DateTime.UtcNow.Date.AddHours(11);
        ...
        Availability randomAvailability = new Availability
        {
        UserName = "Test Availability",
        UserEmail = "test@email.com",
        AvailabilitySlots =
        ...
        using (var context = new LineUpContext(options))
        {
            ...
            var availabilityDto = new AvailabilityCreateDto
            {
                AvailabilitySlots = randomAvailability.AvailabilitySlots,
                UserName = randomAvailability.UserName,
                UserEmail = randomAvailability.UserEmail,
                Preferences = randomAvailability.Preferences,
                FormAnswers = randomAvailability.FormAnswers,
            };
